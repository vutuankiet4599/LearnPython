...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round((df[df['education'] == 'Bachelors'].shape[0] / float(df.shape[0])) * 100, 1)
    ...
    rich_percentage = round((df[(df['hours-per-week'] == 1) & (df['salary'] == '>50K')]['hours-per-week'].count() / num_min_workers) * 100, 1)

    # What country has the highest percentage of people that earn >50K?
    country_percent = df[df['salary'] == '>50K']['native-country'].value_counts() / df['native-country'].value_counts()
    highest_earning_country =country_percent.idxmax()
    highest_earning_country_percentage = round(country_percent[highest_earning_country] * 100, 1)
